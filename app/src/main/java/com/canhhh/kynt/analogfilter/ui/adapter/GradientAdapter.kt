...
class GradientAdapter(var mContext: Context, var mFilters: List<MyFilter>, var mClickAdapter: OnClickAdapter)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (checkClick % 2 == 0) {
            MainActivity.share().alphaGradientSeekBar.visibility = View.VISIBLE
        } else {
            MainActivity.share().alphaGradientSeekBar.visibility = View.GONE
        }

        if (holder.adapterPosition > UnlockManager.getUnLock(mContext, UnlockManager.UNLOCK_GRADIENT)) {
